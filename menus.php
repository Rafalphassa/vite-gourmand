...
if(!empty($_GET['prix_max']))     { $where[] = "m.prix_par_personne <= ?";      $params[] = $_GET['prix_max']; }
if(!empty($_GET['prix_min']))     { $where[] = "m.prix_par_personne >= ?";      $params[] = $_GET['prix_min']; }
if(!empty($_GET['theme_id']))     { $where[] = "m.theme_id = ?";                $params[] = $_GET['theme_id']; }
if(!empty($_GET['regime_id']))    { $where[] = "m.regime_id = ?";               $params[] = $_GET['regime_id']; }
if(!empty($_GET['nb_personnes'])) { $where[] = "m.nombre_personne_minimum <= ?"; $params[] = $_GET['nb_personnes']; }
            ...
            <form method="GET">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" name="prix_min" class="form-control" value="<?php echo htmlspecialchars($_GET['prix_min'] ?? ''); ?>">
                    </div>
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="theme_id" class="form-select">
                            ...
                            <?php foreach($themes as $t): ?>
                                <option value="<?php echo $t['theme_id']; ?>" <?php echo (($_GET['theme_id'] ?? '') == $t['theme_id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="regime_id" class="form-select">
                            ...
                            <?php foreach($regimes as $r): ?>
                                <option value="<?php echo $r['regime_id']; ?>" <?php echo (($_GET['regime_id'] ?? '') == $r['regime_id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" name="nb_personnes" class="form-control" value="<?php echo htmlspecialchars($_GET['nb_personnes'] ?? ''); ?>">
                    </div>
                    ...
                    <div class="col-md-2 d-flex gap-2">
                        ...
                        <a href="/vite-gourmand/menus.php" style="background:#F0EDE4; color:#1A5F7A; border-radius:25px; padding:10px 15px; font-weight:600; text-decoration:none; white-space:nowrap;">
                            Reset
                        </a>
                    </div>
        ...
        <div class="row g-4">
            ...
            <?php else: ?>
                ...
                <div class="col-md-6 col-lg-4">
                    <div style="background:#fff; border-radius:15px; overflow:hidden; height:100%; box-shadow:0 3px 20px rgba(0,0,0,0.06); display:flex; flex-direction:column;">
                        ...
                        <div style="height:200px; background:#F0EDE4; display:flex; align-items:center; justify-content:center; overflow:hidden;">
                            <?php if($photo): ?>
                                <img src="/vite-gourmand/uploads/plats/<?php echo htmlspecialchars($photo); ?>" style="width:100%; height:100%; object-fit:cover;">
                            <?php else: ?>
                        ...
                        <div style="padding:25px; flex:1; display:flex; flex-direction:column;">
                            ...
                            <p style="color:#666; font-size:0.9rem; flex:1;">
                                <?php echo htmlspecialchars(substr($m['description'], 0, 120)); ?>...
                            </p>
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <div>
                                    ...
                                    <div style="color:#999; font-size:0.8rem;">
                                        À partir de <?php echo $m['nombre_personne_minimum']; ?> personnes
                                    </div>
                                </div>
                                <a href="/vite-gourmand/menu-detail.php?id=<?php echo $m['menu_id']; ?>" style="background:#1A5F7A; color:#fff; border-radius:25px; padding:8px 20px; font-weight:600; text-decoration:none; font-size:0.9rem;">
                                    Voir le menu
                                </a>
                            </div>

                            <?php if($m['quantite_restante'] == 0): ?>
                                <div style="background:#f8d7da; color:#721c24; border-radius:8px; padding:8px 12px; font-size:0.8rem; margin-top:10px;">Complet</div>
                            <?php elseif($m['quantite_restante'] <= 3): ?>
                                <div style="background:#fff3cd; color:#856404; border-radius:8px; padding:8px 12px; font-size:0.8rem; margin-top:10px;">Plus que <?php echo $m['quantite_restante']; ?> commande(s) disponible(s)</div>
                            <?php endif; ?>
