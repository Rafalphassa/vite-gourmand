...
$imgs = $pdo->prepare("SELECT * FROM image_menu WHERE menu_id = ? ORDER BY ordre ASC");
$imgs->execute([$id]);
$images = $imgs->fetchAll();

// Récupération des plats avec leurs allergènes
$plats = $pdo->prepare("
        ...
        <div class="row mt-4 g-5">
            ...
            <div class="col-lg-7">
                ...
                foreach($types as $type => $label):
                    $platsFiltres = array_filter($plats, fn($p) => $p['type_plat'] === $type);
            ...
            <div class="col-lg-5">
                ...
                ">
                    ...
                    <p style="color:#666; font-size:0.9rem; margin:10px 0 20px;">
                        Minimum <?php echo $menu['nombre_personne_minimum']; ?> personnes
                    </p>
                    ...
                    <?php elseif($menu['quantite_restante'] <= 3): ?>
                    <div style="background:#fff3cd; color:#856404; border-radius:8px; padding:12px; margin-bottom:15px; font-size:0.9rem;">
                        Plus que <?php echo $menu['quantite_restante']; ?> commande(s) disponible(s).
                    </div>
                    <?php endif; ?>

                    <!-- CONDITIONS bien en évidence comme demandé dans l'énoncé -->
                    ...
                    <?php if($menu['quantite_restante'] > 0): ?>
                        <?php if(isset($_SESSION['utilisateur_id'])): ?>
                            <a href="/vite-gourmand/commande.php?menu_id=<?php echo $menu['menu_id']; ?>" style="
